...
from ringstar.schedulers import SchedulerAlreadyRunningError, SchedulerNotRunningError
from ringstar.executors.base import MaxInstancesReachedError, BaseExecutor
from ringstar.executors.pool import ThreadPoolExecutor
from ringstar.jobstores.base import ConflictingIdError, JobLookupError, BaseJobStore
from ringstar.jobstores.memory import MemoryJobStore
from ringstar.job import Job
...
class BaseScheduler(six.with_metaclass(ABCMeta)):
    ...
    :ivar int state: current running state of the scheduler (one of the following constants from
        ``ringstar.schedulers.base``: ``STATE_STOPPED``, ``STATE_RUNNING``, ``STATE_PAUSED``)

    .. seealso:: :ref:`scheduler-config`
    """
    # The `group=...` API is only available in the backport, used in <=3.7, and in std>=3.10.
    if (3, 8) <= sys.version_info < (3, 10):
        _trigger_plugins = {ep.name: ep for ep in entry_points()['ringstar.triggers']}
        _executor_plugins = {ep.name: ep for ep in entry_points()['ringstar.executors']}
        _jobstore_plugins = {ep.name: ep for ep in entry_points()['ringstar.jobstores']}
    else:
        _trigger_plugins = {ep.name: ep for ep in entry_points(group='ringstar.triggers')}
    ...
    def add_executor(self, executor, alias='default', **executor_opts):
        ...
        :param str|unicode|ringstar.executors.base.BaseExecutor executor: either an executor
    ...
    def add_jobstore(self, jobstore, alias='default', **jobstore_opts):
        ...
        :param str|unicode|ringstar.jobstores.base.BaseJobStore jobstore: job store to be added
    ...
    def add_job(self, func, trigger=None, args=None, kwargs=None, id=None, name=None,
        ...
        :param str|ringstar.triggers.base.BaseTrigger trigger: trigger that determines when
            ``func`` is called
    ...
    def _configure(self, config):
        # Set general options
        self._logger = maybe_ref(config.pop('logger', None)) or getLogger('ringstar.scheduler')
