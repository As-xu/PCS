...
def set_threadpool_executor_shrinkable(min_works=1, keep_alive_time=5):
    DynamicThreadPoolExecutor.MIN_WORKERS = min_works
    DynamicThreadPoolExecutor.KEEP_ALIVE_TIME = keep_alive_time


class DynamicThreadPoolExecutor(Executor, BaseConcurrentPool):
...
class _CustomThread(threading.Thread):
    ...
    def __init__(self, executorx: DynamicThreadPoolExecutor):
        super().__init__()
...
def show_current_threads_num(sleep_time=600, process_name='', block=False, daemon=True):
    ...
    def _show_current_threads_num():
        ...
        while True:
            _log.error(f'{process_name} {os.getpid()} 进程 的 总线程数量是 -->  {threading.active_count()}')
            time.sleep(2)
...
if __name__ == '__main__':
    ...
    def f1(a):
        time.sleep(1)  # 可修改这个数字测试多线程数量调节功能。
        print(f'{a} 。。。。。。。')
        return a * 10
        # raise Exception('抛个错误测试')  # 官方的不会显示函数出错你，你还以为你写的代码没毛病呢。


    pool = DynamicThreadPoolExecutor(1)
